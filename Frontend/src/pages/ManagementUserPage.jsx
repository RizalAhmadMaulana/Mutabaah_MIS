...
const ManagementUserPage = () => {
  ...
  const headers = ["#", "Username", "Nama Lengkap", "No Telp", "Email", "Role", "Aksi"];
  
  const rowData = [
    {
      id: 1, username: "Rizal", name: "Rizal Ahmad M", telp: "089676440508", email: "rizal@gmail.com", role: "WALI MURID"
    },
    {
      id: 2, username: "AdminMIS", name: "Administrator MIS", telp: "081234567890", email: "admin@mis.com", role: "ADMINISTRATOR"
    },
    {
      id: 3, username: "UstadzAli", name: "Ustadz Ali Ridho", telp: "081987654321", email: "ali@musyif.com", role: "MUSYIF"
    },
    {}, {}, {}, {}, {}, {}, {} // Mockup baris kosong
  ];

  // Helper untuk Warna Role Badge sesuai desain
  const renderRoleBadge = (role) => {
    let bgClass = "bg-gray-500";
    if (role === "ADMINISTRATOR") bgClass = "bg-[#1B4332]"; // Hijau Gelap
    else if (role === "MUSYIF") bgClass = "bg-[#5294A9]"; // Teal
    else if (role === "WALI MURID") bgClass = "bg-[#007BFF]"; // Biru

    return (
      <span className={`${bgClass} text-white px-3 py-1.5 rounded-[6px] text-[0.75rem] font-[700] uppercase tracking-wide min-w-[110px] inline-block text-center shadow-sm`}>
        {role}
      </span>
    );
  };
  ...
  const renderTableBody = () => {
    return rowData.map((row, idx) => (
      <tr key={idx} className="even:bg-[#f2f2f2] hover:bg-slate-100 transition-colors">
        {row.id ? (
          <>
            <td className="border border-black px-3 py-3 text-center h-[50px] whitespace-nowrap">{row.id}</td>
            <td className="border border-black px-3 py-3 text-center whitespace-nowrap font-medium">{row.username}</td>
            <td className="border border-black px-3 py-3 text-center whitespace-nowrap">{row.name}</td>
            <td className="border border-black px-3 py-3 text-center whitespace-nowrap">{row.telp}</td>
            <td className="border border-black px-3 py-3 text-center whitespace-nowrap">{row.email}</td>
            <td className="border border-black px-3 py-3 text-center whitespace-nowrap">
          ...
          headers.map((_, cIdx) => <td key={cIdx} className="border border-black px-2 py-2 h-[50px]">&nbsp;</td>)
        )}
      </tr>
    ));
  };
  ...
  return (
    <DashboardLayout title="Management User">
      ...
      {/* MODALS */}
      {activeModal === 'add' && <FormUserModal mode="add" onClose={handleClose} onSave={handleSaveAdd} />}
      {activeModal === 'edit' && <FormUserModal mode="edit" onClose={handleClose} onSave={handleSaveEdit} />}
