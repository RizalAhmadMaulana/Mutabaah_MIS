  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const resKelas = await axios.get("https://api-risalah.mentariku.org/api/academic/kelas/", { headers });
  ...
  const fetchByKelas = async (namaKelas) => {
    ...
    try {
      ...
      const [resSiswa, resGuru] = await Promise.all([
        axios.get(`https://api-risalah.mentariku.org/api/siswa/?kelas=${namaKelas}`, { headers }),
        axios.get(`https://api-risalah.mentariku.org/api/guru/?kelas=${namaKelas}`, { headers })
      ]);
...
export const ImportExcelModal = ({ onClose, onSuccess }) => {
  ...
  const handleImport = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post("https://api-risalah.mentariku.org/api/academic/hafalan/import/", formData, { headers: { Authorization: `Bearer ${token}` } });
