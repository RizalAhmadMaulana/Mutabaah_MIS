...
// --- KOMPONEN RADIO BUTTON CUSTOM (Untuk Jenis Kelamin) ---
...
export const FormSiswaModal = ({ mode = "add", onClose, onSave }) => {
  return (
    <ModalWrapper 
    ...
    >
      <div className="space-y-4">
        <ModalInput label="Nama Lengkap Siswa" placeholder="Masukkan Nama Siswa" value={mode === "edit" ? "Rizal Ahmad M" : ""} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ModalInput label="NISN" placeholder="Masukkan NISN" value={mode === "edit" ? "12345678" : ""} />
            <ModalInput label="Kelas" type="select" options={["-- Pilih Kelas --", "1 A", "1 B"]} value={mode === "edit" ? "1 A" : ""} />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ModalInput label="Tempat, Tanggal Lahir" placeholder="Contoh: Semarang, 01/01/2010" value={mode === "edit" ? "Semarang, 01/01/2010" : ""} />
            <ModalInput label="No Telp Orangtua" placeholder="08xxxxxxxxxx" value={mode === "edit" ? "089676440508" : ""} />
        </div>
...
export const ConfirmModal = ({ type = "save", onClose, onConfirm }) => {
  ...
  const titleText = isDelete ? "Hapus Siswa?" : (type === "edit" ? "Konfirmasi Perubahan" : "Konfirmasi Data Siswa");
  const titleColor = isDelete ? "text-[#DC3545]" : "text-[#007BFF]";
  const descText = isDelete ? "Apakah Anda yakin ingin menghapus data Siswa ini?" : "Silakan periksa kembali data siswa berikut sebelum disimpan.";
      ...
      <div className="bg-white rounded-[15px] w-full max-w-[450px] p-6 md:p-8 text-center shadow-2xl animate-[zoomIn_0.3s_ease-out] overflow-y-auto max-h-[90vh]">
        ...
        {!isDelete && (
          <div className="bg-slate-50 p-4 rounded-lg text-left mx-auto mb-8 border border-slate-200 text-sm w-full">
            ...
            <div className="flex mb-2"><span className="w-[100px] font-bold text-slate-700 shrink-0">Kelas</span><span>: 1 A</span></div>
            <div className="flex"><span className="w-[100px] font-bold text-slate-700 shrink-0">No Telp</span><span>: 089676440508</span></div>
          </div>
        )}
